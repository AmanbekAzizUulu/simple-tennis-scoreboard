...
public class Game {
    ...
    public void scorePointTo(Player player) {
        if (firstPlayer.equals(player)) {
            setNextGamePoint(firstPlayer);
        } else if (secondPlayer.equals(player)) {
            setNextGamePoint(secondPlayer);
        }
    }
    ...
    private void setNextGamePoint(Player player) {
        ...
        if ((table.get(firstPlayer) == GamePoint.FORTY && table.get(secondPlayer) == GamePoint.LOVE) ||
                (table.get(firstPlayer) == GamePoint.FORTY && table.get(secondPlayer) == GamePoint.FIFTEEN) ||
                (table.get(firstPlayer) == GamePoint.FORTY && table.get(secondPlayer) == GamePoint.THIRTY)) {
            if (player.equals(firstPlayer)) {
                System.out.println("First Player Wins");
        ...
        if ((table.get(secondPlayer) == GamePoint.FORTY && table.get(firstPlayer) == GamePoint.LOVE) ||
                (table.get(secondPlayer) == GamePoint.FORTY && table.get(firstPlayer) == GamePoint.FIFTEEN) ||
                (table.get(secondPlayer) == GamePoint.FORTY && table.get(firstPlayer) == GamePoint.THIRTY)) {
            if (player.equals(secondPlayer)) {
                System.out.println("Second Player Wins");
    ...
    // временное решение / для демонстрационных целей
    public Map<Player, GamePoint> getTable() {
        return table;
    }

    // тут хочу привязать очки к состоянию игры — насколько целесообразно пока не могу определить
